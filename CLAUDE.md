...
- `checker.yaml` — the arena `checkers/*.yaml` (validated against `schemas/checker.json`): `build` self-hosts `vowc` then builds the kernel; `run` enforces `ulimit -v 32G` and maps a Vow-runtime OOM to an error (exit 3) so it isn't mistaken for a reject.
